...
/**
  ...
  * This experiment compares the runtime of various bandits w.r.t. an increasing number of arms
  */
object BanditScalabilityK extends BanditExperiment {
  val attributes = List("bandit","dataset","npairs", "nbatches", "scalingstrategy","k","gain","cputime", "iteration")

  val data: Vector[ExternalDataRefResample] = Vector(
    bioliq_1wx10_1000,
    bioliq_1wx20_1000,
    bioliq_1wx50_1000,
    bioliq_1wx100_1000,
    ...
    for {
      ...
      for {
        ...
        for{
          ...
          // Hot fix because it takes too much time for my experiment
          val reps = if(bandit.name.contains("Exp3")) 1 else nRep
