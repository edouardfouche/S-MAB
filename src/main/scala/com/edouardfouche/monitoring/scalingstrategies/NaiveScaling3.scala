...
/**
  * Naive Scaling.
  * Decrement L_t by a 1 if the observed efficiency SO FAR is lower than delta, otherwise increment it by 1
  *
  ...
  *
  * @note the main difference is that uses the sum of weights and counts
  */
case class NaiveScaling3(lmin: Int, lmax: Int, delta: Double) extends ScalingStrategy {
  val gamma = 0.0
  val name=s"Naive3-$lmin-$lmax-$gamma-$delta"
  var confidence=1.0
  var k = lmax
  def scale(rewards: Array[Double], indexes: Array[Int], sums: Array[Double], counts: Array[Double], t:Double): Int = {
    ...
    if(indexes.map(x => sums(x)/counts(x)).sum / Lt < delta) {
